  ...
  <div class="w-full max-w-md">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
      ...
      @php
        ...
        $supabaseAnon = config('services.supabase.anon_key', '');
      @endphp
      <button type="button" id="google-btn"
         onclick="startGoogleSignIn()"
         class="flex items-center justify-center gap-3 w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition mb-5 disabled:opacity-60 disabled:cursor-not-allowed">
...
function startGoogleSignIn() {
    ...
    btn.disabled = true;
    lbl.textContent = 'Redirecting to Google...';
    ...
    client.auth.signInWithOAuth({
        provider: 'google',
        options: {
            redirectTo: '{{ route("auth.google.callback") }}'
        }
    }).catch(function (err) {
        btn.disabled = false;
        lbl.textContent = 'Continue with Google';
        alert('Could not start Google sign-in: ' + (err.message || 'Unknown error'));
    });
}
</script>
@endpush
